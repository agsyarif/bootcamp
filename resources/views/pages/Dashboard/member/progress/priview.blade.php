...
@section('content')
    ...
    <div class="mt-2 mx-10 p-2 pl-5 bg-white sm:rounded-lg grid grid-cols-6">
                    ...
                    <li class="flex justify-between gap-x-6 py-5">
                        <div class="flex gap-x-4">
                            <div class="min-w-0 flex-auto">
                                <a href="#" onclick="activateTab(event, '{{$key}}')" class="linkTabs py-4 px-6 bg-gray-200 text-gray-800 font-semibold">{{$loop->iteration}} .{{$score->exam->title}}</a>
                            </div>
                        </div>
                    </li>
                @endforeach
        ...
        <div class="col-span-4 ml-4">
            @foreach ($aksesCourse->score as $key => $score)
                <span id="tab-content-{{$key}}" class="tab-content">
                    @php
